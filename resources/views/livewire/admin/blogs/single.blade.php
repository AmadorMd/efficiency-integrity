<article class="flex flex-row items-center mt-5">
    <div class="w-1/3"> 
        <img class="w-full" src="{{ Storage::disk('public')->url($entry->image_url) }}" alt="">
    </div>
    <div class="w-2/3 pl-5">
        ...
        <div class="flex flex-row items-center justify-start mt-4 gap-3">
            <a href="{{ route('blogs.entry', [
                'entry' => $entry
            ]) }}" class="bg-primary text-xs px-3 py-1 text-white tracking-wide rounded-full font-bold">
                Ver entrada
            </a>
            <a href="{{ route('blogs.edit', $entry) }}" class="bg-indigo-400 text-xs px-3 py-1 text-white tracking-wide rounded-full font-bold">
                Editar
            </a>
